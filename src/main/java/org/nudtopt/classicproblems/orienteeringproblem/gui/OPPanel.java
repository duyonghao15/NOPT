package org.nudtopt.classicproblems.orienteeringproblem.gui;
...
public class OPPanel extends PlotPanel {
    ...
    public void paintComponent(Graphics g){
        ...
        Solution OPSolution = (Solution) solution;
        ...
        Double yMin = null;
        Double yMax = null;
        for(Vertex vertex : OPSolution.getVertexList()) {
        ...
        // 3. 画线
        Vertex startVertex = OPSolution.getVertexList().get(0);
        for(int i = 0 ; i < OPSolution.getTourList().size() ; i ++) {               // 遍历路径
            Tour tour = OPSolution.getTourList().get(i);
            List<Vertex> visitedVertexList = new ArrayList<>(tour.getVertexList()); // 按顺序访问的顶点
            visitedVertexList.add(0, startVertex);                            // 把起点加上
            int colorIndex = (i + 1) % TangoColorFactory.SEQUENCE_1.length;         // 颜色周期性递增变化
            g.setColor(TangoColorFactory.SEQUENCE_1[colorIndex]);                   // 逐一赋颜色
            // 遍历顶点
            for(int j = 0 ; j < visitedVertexList.size() ; j ++) {
                Vertex vertex_1 = visitedVertexList.get(j);
                Vertex vertex_2 = j < visitedVertexList.size() - 1 ? visitedVertexList.get(j + 1) : visitedVertexList.get(0);
                double x_1 = vertex_1.getX();
                double y_1 = vertex_1.getY();
                double x_2 = vertex_2.getX();
                double y_2 = vertex_2.getY();
